...
![Project Image](https://github.com/kleenpulse/ai_crypto_coterie/blob/master/public/coterie-banner.png)
...
- Helvetica ---> usage "font-Helvetica"
...
Coterie is open to contributions, but I recommend creating an issue or replying in a comment to let us know what you are working on first that way we don't overwrite each other.
...
5. After making changes, format codes with `yarn run format`
...
10. Create a pull request to the `master`.
...
Under no circumstances should you merge a pull requests on a specific branch to the `master` branch
...
| build    | Builds                   | Changes that affect the build system or external dependencies (example scopes: gulp, broccoli, yarn)        |
